...
router.put('/', (req, res) => {
  ...
  db.query(
    `UPDATE pokemons SET "${attribute}"='${newValue}' WHERE id=${pokemonId};`
  )
    .then(data => res.send(data))
    .catch(err => res.status(500).send(err));
});
...
router.get('/wishlistSeedTest', (req, res) => {
  let promiseArr = [];
  for (let i = 0; i < 25; i++) {
    promiseArr.push(
      new Promise((resolve, reject) => {
        let randomPokemon = Math.floor(Math.random() * Math.floor(800) + 1);
        resolve(
          db.query(
            `UPDATE pokemons SET “wishList”=true WHERE id=${randomPokemon};`
          )
        );
      })
    );
